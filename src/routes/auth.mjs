...
router.post('/api/v1/auth', 
  ...
  async (request, response) => {
    ...
    const  { User } = models;
    const user = await User.findOne({where: {user_name: username, is_active: true}});
    ...
    return response.status(200).send({ message: "Authentication successful", token: user_jwt_token });
